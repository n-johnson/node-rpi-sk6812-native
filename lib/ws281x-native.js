...
var remap = (function () {
    ...
    return function remap(data, indexMapping) {
        if (!_tmpData || _tmpData.length !== data.length) {
            _tmpData = new Uint32Array(data.length);
        }

        for (var i = 0; i < data.length; i++) {
            _tmpData[i] = data[indexMapping[i]];
        }
...
// Gamma correction should only be used for 24-bit RGB strips,
// not 32-bit WRGB strips
var _useGammaCorrection = true;
...
ws281x.init = function (numLeds, options) {
    ...
    _useGammaCorrection = options.gammaCorrection
        ? (options.strip_type !== STRIP_TYPES.SK6812W)
        : false;
...
ws281x.render = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var color_data = _useGammaCorrection ? gammaCorrect(data[i]) : data[i];
        _outputBuffer.writeUInt32LE(color_data, 4 * i);
    }
