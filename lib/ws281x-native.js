 ...
 *
 * @type function(Uint32Array, Array.<Number>)
 */
var gammaCorrect = (function () {
    ...
    return function gammaCorrect(data) {
        for (var i = 0; i < data.length; i++) {
            data[i] = (_gammaTable[data[i] & 0xff]
                | (_gammaTable[(data[i] >> 8) & 0xff] << 8)
                | (_gammaTable[(data[i] >> 16) & 0xff] << 16)
                | (_gammaTable[(data[i] >> 24) & 0xff] << 24)) >>> 0
        }
    };
}());
...
var remap = (function () {
    return function remap(data, indexMapping) {
        var _tmpData = Uint32Array.from(data);
        for (var i = 0; i < data.length; i++) {
...
/**
 * convert pixels from rgb to rgbw.
 *
 * @type function(Uint32Array, Array.<Number>)
 */
...
ws281x.render = function (data) {
    ...
    if (_indexMapping || _rgbToRgbw || _useGammaCorrection) {
